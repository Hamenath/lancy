...
import { SlidersHorizontal, RefreshCw, Search, Plus, ArrowRight } from "lucide-react";
...
export default function Projects() {
  ...
  const queryMinBudget = searchParams.get("minBudget") ? Number(searchParams.get("minBudget")) : 0;
  const queryMaxBudget = searchParams.get("maxBudget") ? Number(searchParams.get("maxBudget")) : 50000;
  const querySort = searchParams.get("sort") || "newest";

  const [searchTerm, setSearchTerm] = useState(queryQ);
  const [maxBudget, setMaxBudget] = useState(queryMaxBudget);
    ...
    async function loadProjects() {
      try {
        ...
        const result = await searchService.searchProjects({
          q: queryQ,
          minBudget: queryMinBudget > 0 ? queryMinBudget : undefined,
          maxBudget: queryMaxBudget < 50000 ? queryMaxBudget : undefined,
          sort: querySort,
        });
        ...
        <section className="relative pt-32 pb-12 border-b border-neutral-900 bg-neutral-950/40">
          <div className="w-full px-4 md:px-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            {(user as any)?.role === "CLIENT" && (
              <Link
                to="/create-project"
                className="px-5 py-3 bg-white text-black font-bold text-xs hover:bg-neutral-200 transition-colors flex items-center gap-2 rounded-none"
              >
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <aside className="w-full lg:w-64 shrink-0 border border-neutral-900 bg-neutral-950/20 p-5 rounded-none space-y-6 h-fit">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-neutral-300">Max Budget: ${maxBudget.toLocaleString()}</label>
                <input
                  type="range"
                  min="500"
                  max="50000"
            ...
            <main className="flex-1">
              {loading ? (
                ...
                <div className="border border-neutral-900 bg-neutral-950/20 p-12 text-center rounded-none space-y-3">
                  <p className="text-base font-bold text-neutral-300">No active projects found matching your criteria.</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {projects.map((project) => (
                    <div
                      key={project.id}
                      className="border border-neutral-900 bg-neutral-950/40 p-6 flex flex-col justify-between hover:border-neutral-800 transition-colors rounded-none space-y-4"
                    >
                      <div className="space-y-2">
                        <div className="flex justify-between items-start gap-4">
                          <h3 className="text-base font-bold text-white hover:text-brand-primary transition-colors">
                      ...
                      {project.skills && project.skills.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {project.skills.slice(0, 4).map((s: string) => (
                            <span key={s} className="bg-neutral-900 text-neutral-400 text-[10px] px-2 py-0.5 rounded-none">
                              {s}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="pt-3 border-t border-neutral-900 flex justify-between items-center text-[10px] text-neutral-500">
                        <span>Posted by {project.clientName || "Client"}</span>
                        <Link
                          to={`/projects/${project.id}`}
                          className="text-white hover:text-brand-primary font-bold flex items-center gap-1 transition-colors"
                        >
